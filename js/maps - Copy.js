jQuery(document).ready(function($) {
    ...
    getAllRoutesForMap();
	loadDatesIntoDropdownBox();
    loadRoutesIntoDropdownBox();
    
    $("#routeSelect").change(function() {
	//	alert($('#routeSelect').val()+$('#dateSelect').val());
        if (hasMap()) {
    ...
    function loadRoutes(json) {        
        if (json.length == 0) {
            showPermanentMessage('No hay actualizaciones disponibles');
        }
        else {
            // create the first option of the dropdown box
            var option = document.createElement('option');
            option.setAttribute('value', '0');
            option.innerHTML = 'Seleccione Vendedor...';
            ...
            $(json.routes).each(function(key, value){
                ...
                option.innerHTML = $(this).attr('userName') ;
                routeSelect.appendChild(option);
            });
	...
	function loadDates(json) {        
        if (json.length == 0) {
            ...
            var option = document.createElement('option');
            option.setAttribute('value', '0');
            option.innerHTML = 'Seleccione Fecha...';
            ...
            // iterate through the routes and load them into the dropdwon box.
            $(json.dates).each(function(key, value){
                var option = document.createElement('option');
                option.setAttribute('value', 'dateid=' + $(this).attr('dateid'));

                sessionIDArray.push($(this).attr('dateid'));

                option.innerHTML = $(this).attr('dateid') ;
                dateSelect.appendChild(option);
            });
            ...
            showPermanentMessage('Seleccione Fecha');
        }
    }
    ...
    function loadGPSLocations(json) {
        ...
        if (json.length == 0) {
            ...
            if (map.id == 'map-canvas') {
			  ...
			  // var openStreetMapsURL = ('https:' == document.location.protocol ? 'https://' : 'http://') +
                // '{s}.tiles.mapbox.com/v3/{id}/{z}/{x}/{y}.png';
              ...
              //  var bingMapsLayer = new L.BingLayer("AnH1IKGCBwAiBWfYAHMtIfIhMVybHFx2GxsReNP5W0z6P8kRa67_QwhM4PglI9yL");
                var googleMapsLayer = new L.Google('ROADMAP');
				 var googleMapsLayerSat = new L.Google('HYBRID');
                ...
                gpsTrackerMap.addControl(new L.Control.Layers({
                    ...
                    'Google Maps':googleMapsLayer,
					 'Google Maps Satellite':googleMapsLayerSat
                ...
                $(json.locations).each(function(key, value){
                  ...
                  //  var dir=displayDirAprox(latitude,longitude);
    ...
    function createMarker(latitude, longitude, speed, direction, distance, locationMethod, gpsTime,
        ...
        if (finalLocation) {
            lastMarker = "</td></tr><tr><td align=left>&nbsp;</td><td><b>Ultima Lozalizacion</b></td></tr>";
        }
        ...
        var popupWindowText = "<table border=0 style=\"font-size:95%;font-family:arial,helvetica,sans-serif;color:#000;\">" +
           ...
           // "<tr><td align=right>Distancia:&nbsp;</td><td>" + distance +  " km</td><td>&nbsp;</td></tr>" +
              "<tr><td align=right> </td></tr>" +
            "<tr><td align=right></td><td>&nbsp;</td></tr>" +
            "<tr><td align=right>Tiempo:&nbsp;</td><td colspan=2>" + gpsTime +  "</td></tr>" +
            "<tr><td align=right>Nombre:&nbsp;</td><td>" + userName + "</td><td>&nbsp;</td></tr>" +
            "<tr><td align=right>Altura:&nbsp;</td><td>" + accuracy + " metros</td><td>&nbsp;</td></tr></table>";
	...
	function displayDirAprox(latitude, longitude) {
        var lat = parseFloat(latitude);
        var lng = parseFloat(longitude);
        var latlng = new google.maps.LatLng(lat, lng);
        var reverseGeocoder = new google.maps.Geocoder();
		var reverseGeocoderResult="nohay";
        ...
        reverseGeocoder.geocode({'latLng': latlng}, function(results, status) {
            if (status == google.maps.GeocoderStatus.OK) {
                ...
                if (results[1]) {
					...
					//alert (reverseGeocoderResult);
                    return reverseGeocoderResult;
                }
            } else {
                return "N/A";
            }
    ...
    function turnOnAutoRefresh() {
        showMessage('Auto Actualizar (1 min)'); 
